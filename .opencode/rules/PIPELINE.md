...
| code-generator | stack-compliance-guard, validation-engine, wiki-engine, design-engine (retroalimentación) | `modules/*`, `core/*` (incl. `core/sync.js`), `index.html` (+ `neutralino.config.json` en Professional/Business, + `capacitor.config.json` en Business) |

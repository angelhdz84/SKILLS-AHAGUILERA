...
if ((Test-Path $iaFullPath)) {
    ...
    $wasmFiles = @(
        ...
        @("sql-wasm.wasm", "https://cdn.jsdelivr.net/npm/sql.js@1.10.0/dist/sql-wasm.wasm"),
        @("sql-wasm.js", "https://cdn.jsdelivr.net/npm/sql.js@1.10.0/dist/sql-wasm.js")
    )
    ...
    if (-not (Test-Path $pdfPath) -or $Force) {
        ...
        try {
            Invoke-WebRequest -Uri "https://cdn.jsdelivr.net/npm/pdfjs-dist@4.8.69/build/pdf.min.js" -OutFile $pdfPath -UseBasicParsing
            $pdfWorker = Join-Path $assetsDir "pdf.worker.min.js"
            Invoke-WebRequest -Uri "https://cdn.jsdelivr.net/npm/pdfjs-dist@4.8.69/build/pdf.worker.min.js" -OutFile $pdfWorker -UseBasicParsing
        } catch {
    ...
    # Tesseract.js UMD (OCR) - los datos de lenguaje 'spa' se descargan en runtime desde jsDelivr
    $tesPath = Join-Path $assetsDir "tesseract.min.js"
    if (-not (Test-Path $tesPath) -or $Force) {
        Write-Output "[Full+] Descargando Tesseract.js..."
        try {
            Invoke-WebRequest -Uri "https://cdn.jsdelivr.net/npm/tesseract.js@5.1.1/dist/tesseract.min.js" -OutFile $tesPath -UseBasicParsing
        } catch {
            Write-Output "[ERR]  No se pudo descargar Tesseract.js: $_"
        }
    } else {
        Write-Output "[OK]  Tesseract.js presente"
    }
...
Write-Output ""
Write-Output "  Full+  (~33MB) - Embeddings ONNX + OCR + parsers + SQLite FTS5"

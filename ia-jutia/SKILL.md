---
name: ia-jutia
description: Mini IA offline-first como plugin auto-contenido. Lite: FlexSearch + chat conversacional + estadisticas + predicciones + herramientas extensibles. Full (DLC): +ingesta PDF/DOCX/XLSX/CSV/MD + Transformers.js QA + OCR. Plugin de un solo script, sin modificar core/ ni index.html.
license: MIT
compatibility: Requiere @AGENTS.md y spec validada. Perfil Lite requiere solo FlexSearch (~40KB). Perfil Full requiere Transformers.js + modelos (~233MB descarga unica).
meta:
...
| **Full** | ~233MB | Lite + ingesta documentos + QA + OCR | DLC opcional. Descarga una vez, ruta compartida entre apps |
...
[2] Full (Lite + ingesta documentos + QA + OCR) — +233MB DLC
    Todo lo de Lite + subir PDF/DOCX/XLSX/CSV/MD para consultar.
    OCR automatico en PDFs escaneados.
...
```
⚠️ PERFIL FULL: DLC de ~233MB
  Los modelos se comparten entre apps del mismo equipo
  (una sola descarga, no N×233MB).
...
```
📁 modules/ia-jutia/
├── ia-full.js          → window.iaFull: ingest + QA + Transformers
...
```
templates/plugin/
├── module.js           → Entry point (558 lines)
├── ia-core.js          → window.ia (270 lines)
├── ia-chat.js          → window.ia.chat (473 lines)
├── setup-ia.ps1        → Instalador
├── assets/             → FlexSearch (descargado por setup)
└── tools/
    ├── _registry.js    → window.IA_TOOLS (74 lines)
    └── extraer-factura.js → Extraccion Latam (139 lines)
```
...
- **Transformers.js (Full)**: `pipeline('feature-extraction', ...)` con `dtype: 'q4'`. Worker para no bloquear UI.

...
function Download-IfMissing {
    param([string]$Path, [string]$Url, [int]$MinBytes = 1)
    ...
    try {
        ...
        if ($size -lt $MinBytes) {
            throw "Archivo demasiado pequeno ($size bytes)"
        }
        # Detectar respuesta HTML de error (ej. jsDelivr 404) en archivos pequenos
        if ($size -lt 4096) {
            $head = [System.IO.File]::ReadAllText($Path)
            if ($head -match "<!DOCTYPE|<html") { throw "Respuesta HTML de error (404?)" }
        }
        Write-Output "  [OK]  $Path ($([math]::Round($size/1KB)) KB)"
    } catch {
        $script:dlErrors++
...
$assets = @(
    ...
    @("assets\transformers.min.js", "https://cdn.jsdelivr.net/npm/@huggingface/transformers@4.2.0/dist/transformers.min.js", 1000000),
    @("assets\pdf.min.js", "https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/build/pdf.min.js", 100000),
    @("assets\pdf.worker.min.js", "https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/build/pdf.worker.min.js", 100000),
    @("assets\mammoth.min.js", "https://cdn.jsdelivr.net/npm/mammoth@1.8.0/mammoth.browser.min.js"),
    @("assets\xlsx.full.min.js", "https://cdn.sheetjs.com/xlsx-0.20.2/package/dist/xlsx.full.min.js", 100000),
    @("assets\tesseract.min.js", "https://cdn.jsdelivr.net/npm/tesseract.js@5.1.1/dist/tesseract.min.js", 50000),
    @("assets\worker.min.js", "https://cdn.jsdelivr.net/npm/tesseract.js@5.1.1/dist/worker.min.js", 50000),
    @("assets\tesseract-core-simd.wasm.js", "https://cdn.jsdelivr.net/npm/tesseract.js-core@5.1.0/tesseract-core-simd.wasm.js", 1000000),
    @("assets\tessdata\spa.traineddata.gz", "https://cdn.jsdelivr.net/npm/@tesseract.js-data/spa/4.0.0_best_int/spa.traineddata.gz", 500000),
    @("assets\wasm\ort-wasm-simd-threaded.wasm", "https://cdn.jsdelivr.net/npm/onnxruntime-web@1.20.0/dist/ort-wasm-simd-threaded.wasm", 1000000),
    @("assets\wasm\sql-wasm.wasm", "https://cdn.jsdelivr.net/npm/sql.js@1.10/dist/sql-wasm.wasm", 100000),
    @("assets\wasm\sql-wasm.js", "https://cdn.jsdelivr.net/npm/sql.js@1.10/dist/sql-wasm.js", 30000)
)

foreach ($asset in $assets) {
    $minB = 1
    if ($asset.Count -gt 2) { $minB = $asset[2] }
    Download-IfMissing -Path "$fullDir\$($asset[0])" -Url $asset[1] -MinBytes $minB
}
...
if (-not $SkipModels) {
    $models = @(
        @("models\Xenova\all-MiniLM-L6-v2\onnx\model_quantized.onnx", "https://huggingface.co/Xenova/all-MiniLM-L6-v2/resolve/main/onnx/model_quantized.onnx", 1000000),
    ...
    foreach ($model in $models) {
        $minM = 1
        if ($model.Count -gt 2) { $minM = $model[2] }
        Download-IfMissing -Path "$fullDir\$($model[0])" -Url $model[1] -MinBytes $minM
    }
} else {
...
if ($script:dlErrors -gt 0) {
    Write-Output ""
    Write-Output "  [WARN] $($script:dlErrors) descarga(s) fallaron - revise los mensajes [ERR]"
    Write-Output "         Los ZIPs pueden estar incompletos. Ejecute con -Force para reintentar."
    exit 1
}

# --- 6. Limpiar directorios temporales ---
